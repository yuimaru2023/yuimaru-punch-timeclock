...
export default function LastMonth() {
    ...
    async function dfunc(theMonth,theWorkPlace){
        const employee = await GetEmployee(theWorkPlace)
        const monthData = await GetMonthData(theMonth,theWorkPlace)
    ...
    function ChangeMonth (param) { 
        ...
        console.log(previousMonth,"prev")
        setCurrentDate(previousMonth);
        dfunc(previousMonth.getMonth()+1,workPlace)
    }
    ...
    useEffect( ()=>{
        dfunc(currentDate.getMonth() + 1,workPlace)
    },[])
    useEffect(()=>{
    ...
    return(<>
        ...
        <div className={css.btnBox}>
            ...
            <Link className={css.selected} href="/administrator/ousyu">奥州市</Link>
            <Link className={css.btn} href="/administrator/rikutaka">陸前高田市</Link>
        </div>
